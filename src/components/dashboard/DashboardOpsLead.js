...
function DashboardOpsLead(projectId) {
    ...
    useEffect(async () => {
        if(projectId){
            // let data = dispatch(actions.getActivitySummaryWHProject(Global?.user?.userID, projectId.projectId))
            let data = await dispatch(actions.getActivitySummaryWHProject(3, 3))

            console.log(data)
            setDetailProject(data[0])
        }
    }, []);
